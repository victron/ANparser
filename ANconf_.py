...
top_keywords = {0: {"services charging billing-plan": "BP",
                ...
                1: {"BP": {},
                    "SR": {k[0]: k[0].replace("-", "_")
                           for k in [("admin-state", "m"),
                                     ("priority", "m"),
...
class Element:
    def __init__(self, prefix: str, parameter: str, children: list):
        self.prefix = prefix
...
class Load:
    ...
    def _get_level(self, line: str):
        level = 0
        while line[level] == " ":
            level += 1
        return level

    def _get_prefix(self, line: str):
    ...
    def _parser(self, data):
        for line in data:
            ...
            prefix, parameter = self._get_prefix(line)
            if level == 0 and self.current_level == 0 and self.stack == []:
                newObj = type(top_keywords[0][prefix], (Element,), {})(prefix, parameter, [])
            ...
            if level == self.current_level:
                ...
                try:
                    ...
                    logger.debug("nothing to remove from stack")

                newObj = type(top_keywords[level][type(parent).__name__][prefix], (Element,), {})(prefix, parameter, [])
                getattr(parent, "children").append(newObj)
                self.stack.append(newObj)
            if level > self.current_level:
                parent = self.stack[level - 1]
                newObj = type(top_keywords[level][type(parent).__name__][prefix], (Element,), {})(prefix, parameter, [])
